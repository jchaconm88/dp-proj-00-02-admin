...
export function useEntitySearchIndex(accountId: string | null): {
  ...
  useEffect(() => {
    ...
    async function fetchIndex() {
      try {
        const data = await adminFetch<EntitySearchIndexDoc | null>(
          `/admin/system/entity-search-index?accountId=${encodeURIComponent(aid)}`
        );
        if (cancelled) return;
        setRaw(data);
        setLoading(false);
        ...
        if (cancelled) return;
        console.error("[entity-search] admin fetch error:", err);
        retryRef.current++;
  ...
  return {
    ...
    loading,
    error,
    refetch: useCallback(() => setReloadTick((v) => v + 1), []),
  };
}

export async function rebuildEntitySearchIndex(accountId: string): Promise<{ ok: boolean; summary?: Record<string, number> }> {
  const aid = String(accountId ?? "").trim();
  if (!aid) return { ok: false };
  return adminFetch<{ ok: boolean; summary?: Record<string, number> }>(
    `/admin/system/entity-search-index/rebuild?accountId=${encodeURIComponent(aid)}`,
    { method: "POST" }
  );
}
