...
export function hasPermissionCode(effectivePermissions: string[], action: string, moduleName: string): boolean {
  ...
  return set.has("*") || set.has(`${moduleCode}:${actionCode}`) || set.has(`*:${moduleCode}`);
}
